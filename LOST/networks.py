...
def get_model(arch, patch_size, resnet_dilate, device, trainable=False):
    if "resnet" in arch:
        if resnet_dilate == 1:
...
def dino_forward(dino_model, real_imgs, use_qkv_feats=False):
    feat_out = {}
    ...
    attentions = dino_model.get_last_selfattention(real_imgs)
    handle.remove()
    # Dimensions
    ...
    qkv_feats = None
    if use_qkv_feats:
        qkv_feats = qkv.transpose(2, 3).reshape(3, nb_im, nb_tokens, -1).transpose(0, 1)[:, :, 1:, :]

    return feats, qkv_feats
