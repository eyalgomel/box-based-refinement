...
from bbr.inference.run_inference import inference_bbox_lost
from detr.models.matcher import HungarianMatcher
from detr.models.detr import SetCriterion as DETRLoss
from LOST.networks import dino_forward
from LOST.networks import get_model as get_dino_model
...
def save_models(box_model: nn.Module, dino_model: nn.Module, val_results: float, kwargs: dict):
    save_model = False
    if val_results >= kwargs["best_corloc_lost"]:
        kwargs["best_corloc_lost"] = val_results
        save_model = True
...
def training_step(
    ...
    **kwargs,
    ...
    for batch_idx, inputs in enumerate(pbar):
        ...
        lost_target = [
            {
                "boxes": box_convert(lost_box / real_imgs.shape[-1], "xyxy", "cxcywh").to(device),
                "labels": torch.as_tensor([0]).to(device),  # using only object/no object label
            }
            for lost_box in lost_pred_xyxy
        ]
        ...
        loss_dict = criterion(pred, lost_target)
        ...
        with torch.no_grad():
            ...
            logger.add_scalar(
                "Training/CorLoc_LOST_gt",
                lost_and_gt_corloc.float().mean().item() * 100,
                epoch * len(dataloader) + batch_idx,
            )
...
@hydra.main(version_base=None, config_path="../../config/train", config_name="od_lost")
def main(cfg: DictConfig) -> None:
    ...
    dino_model = get_dino_model("vit_small", 16, 2, device, trainable=True)
    dino_fixed_model = get_dino_model("vit_small", 16, 2, device, trainable=False)
