...
try:
    import moveseg.mae_models
    from moveseg.utils_move import shift, unpatchify, compute_masks
except:
    import mae_models
...
class SegmenterConvHead(nn.Module):
    def __init__(self, upsampling_blocks, in_channels, widths, mask_channels, block_depth):
        super().__init__()
        ...
        layers.append(nn.Conv2d(widths[-1], mask_channels, kernel_size=1, stride=1, padding=0))
...
def build_segmenter_head(args, extractor):
    head_type = "conv_minc128"  # args.segmenter_head
    ...
    if head_type.startswith("conv"):
        ...
        widths = [max(channel_min, int(embed_dim * width_mult ** (i + 1))) for i in range(upsampling_blocks + 1)]
...
class ExtractorSegmenter(nn.Module):
    def __init__(self, feature_extractor, segmenter_head, train_fe=False):
        super().__init__()
        self.feature_extractor = feature_extractor
        self.segmenter_head = segmenter_head
        self.random_segmenter = None
    ...
    def forward(self, x, return_log_imgs=False):
        with torch.no_grad():
            ...
            x_fixed = self.segmenter_head(feat, return_log_imgs=return_log_imgs)
        x = self.random_segmenter(feat, return_log_imgs=return_log_imgs)

        return self.bin_fn(x), self.bin_fn(x_fixed), feat
...
class MAEComposer(nn.Module):
    ...
    def mask2tile(self, mask):
        ...
        return 1 - F.max_pool2d(mask, kernel_size=self.mae_patch_size, stride=self.mae_patch_size).flatten(1)
    ...
    def forward_encoder_full(self, x):
        ...
        if x.shape[-1] != 224 or x.shape[-2] != 224:
            ...
            w, h = x.shape[-2:]  # should be h, w but seems to be swapped in interpolate_pos_encoding
    ...
    def forward(self, x, shifts=None, return_log_imgs=False):
        ...
        if self.mae_as_feature_extractor:
            latent = self.forward_encoder_full(x)  # used for both feature extraction & inpainting
        else:
            latent = x
        ...
        pred_bg, bg_encoder_mask = self.forward_bg_inpaint_diff(latent, bg_inp_mask_, binarize=not self.diff_inp_mask)
...
class MAEExtractorWrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        if h != 224 or w != 224:
            ...
            x = proj(x).flatten(2).transpose(1, 2)
            pos_emb = self.interpolate_pos_encoding(x, h, w)  # this has to be double checked
        else:
            x = mae_model.patch_embed(x)
            pos_emb = mae_model.pos_embed
